    ...
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-[var(--muted)]">התחברו לחשבון SafeTicket שלכם</p>
        </div>

        <form onSubmit={handleSubmit} className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-8">
          {error && (
            <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              {error === 'Invalid login credentials' ? 'אימייל או סיסמה שגויים' : error}
            </div>
          )}
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <input name="password" type={showPassword ? 'text' : 'password'} required placeholder="••••••••" className="w-full rounded-lg border border-[var(--input-border)] bg-[var(--input-bg)] px-4 py-3 text-sm placeholder-[var(--muted)] focus:border-[var(--accent)] focus:outline-none focus:ring-1 focus:ring-[var(--accent)]" />
          ...
          <div className="mt-4 text-end">
            <Link href="/auth/forgot-password" className="text-sm text-[var(--accent-text)] hover:underline">שכחת סיסמה?</Link>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-[var(--muted)]">
          אין לכם חשבון?{' '}<Link href="/auth/signup" className="text-[var(--accent-text)] hover:underline">הרשמו</Link>
        </p>
      </div>
    </div>
  );
